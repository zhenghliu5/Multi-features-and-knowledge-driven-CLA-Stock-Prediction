...
for ss in range(2, len(StockList)-2):
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig("./data/pic/LSTM_M_Training Loss of " + EStockList[ss] + ".png", bbox_inches="tight")
    plt.show()
    ...
    ax.set_title('LSTM_M:Stock price of '+EStockList[ss], size=14, fontweight='bold')
    ...
    plt.savefig("./data/pic/"+" LSTM_M_Stock price of "+EStockList[ss]+".png", bbox_inches="tight")
    plt.show()
    ...
    # 将DataFrame保存到CSV文件
    df.to_csv("./data/results/"+StockList[ss]+"LSTM_M_evaluation_metrics.csv", encoding='utf-8-sig')
