...
def dataloader(file1, file2, file3):
    ...
    entity = []
    relation = []
    with open(file2, 'r', encoding='gbk', errors='ignore') as f1, open(file3, 'r', encoding='gbk', errors='ignore') as f2:
        lines1 = f1.readlines()
        lines2 = f2.readlines()
        for line in lines1:
    ...
    with codecs.open(file1, 'r', encoding='gbk', errors='ignore') as f:
        content = f.readlines()
        for line in content:
...
if __name__ == '__main__':
    ...
    # modify by yourself
    transE = TransE(entity_set, relation_set, triple_list, embedding_dim=300, lr=0.01, margin=1.0, norm=1)
    transE.data_initialise()
    transE.training_run(out_file_title="finance_")
