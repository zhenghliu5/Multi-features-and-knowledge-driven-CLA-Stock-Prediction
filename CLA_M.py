...
for ss in range(2, len(StockList)-1):
    ...
    class CNNLSTMAttentionModel(nn.Module):
        def __init__(self, input_size, window_size, hidden_size):
            ...
            # CNN layer
            self.conv1d = nn.Conv1d(input_size, 64, kernel_size=1)
            self.relu = nn.ReLU()
            ...
            # Attention layer
            self.attention = Attention3D(hidden_size * 2, window_size, single_attention_vector=False)

            # 用于将输入张量展平以进行全连接操作
            self.flatten = nn.Flatten()

            # Fully Connected layer
            self.fc = nn.Linear(window_size * hidden_size * 2, 1)

        def forward(self, inputs):
            x = inputs.permute(0, 2, 1)  # Reshape to (batch_size, input_dims, window_size)
    ...
    class Attention3D(nn.Module):
        def __init__(self, input_size, window_size, single_attention_vector=True):
            ...
            self.dense = nn.Linear(input_size, hidden_size * 2)
    ...
    model = CNNLSTMAttentionModel(input_size, window_size, hidden_size)
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig("./data/pic/CLA_M_Training Loss of " + EStockList[ss] + ".png", bbox_inches="tight")
    plt.show()
    ...
    ax.set_title('CLA_M:Stock price of ' + EStockList[ss], size=14, fontweight='bold')
    ax.set_xlabel("Dates", size=14)
    ax.set_ylabel("Close Prices", size=14)
    plt.vlines(datetime.date(2020, 4, 22), original_min, original_max, linestyles='--', colors='gray',
               label='Train/Test data cut-off')
    plt.savefig("./data/pic/" + " CLA_M_Stock price of " + EStockList[ss] + ".png", bbox_inches="tight")
    plt.show()
    ...
    # 将DataFrame保存到CSV文件
    df.to_csv("./data/results/" + StockList[ss] + "CLA_M_evaluation_metrics.csv", encoding='utf-8-sig')
